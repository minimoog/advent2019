...
func getSegments(input: String) -> [Segment] {
    struct Point {
        let x: Int
        let y: Int
    }

    let segmentStrings = input.components(separatedBy: ",")
    
    let cornersFirst: [Point] = segmentStrings.map {
        let dir = $0.first
        let num = Int($0.dropFirst())!
        
        if dir == "R" {
            return Point(x: num, y: 0)
        } else if dir == "U" {
            return Point(x: 0, y: num)
        } else if dir == "L" {
            return Point(x: -num, y: 0)
        } else {
            return Point(x: 0, y: -num)
    ...
    segments.append(Segment(x1: 0, y1: 0, x2: 0, y2: 0, cumLen: 0))

    for corner in cornersFirst {
        let segment = segments.last!
        
        let nextSegment = Segment(x1: segment.x2,
                                  y1: segment.y2,
                                  x2: segment.x2 + corner.x,
                                  y2: segment.y2 + corner.y,
                                  cumLen: segment.cumLen + abs(corner.x) + abs(corner.y))
...
let first = getSegments(input: firstWireInput)
let second = getSegments(input: secondWireInput)


func checkSegmentIntersect(first: Segment, second: Segment) -> (Int, Int)? {
    
    if first.x1 == first.x2 && second.x1 == second.x2 { return nil }
    if first.y1 == first.y2 && second.y1 == second.y2 { return nil }
    if first.x1 == second.x1 && first.y1 == second.y1 { return nil }
    if first.x2 == second.x2 && first.y2 == second.y2 { return nil }
    
    if first.x1 != first.x2 {
        let x1 = min(first.x1, first.x2)
        let x2 = max(first.x1, first.x2)

        let y1 = min(second.y1, second.y2)
        let y2 = max(second.y1, second.y2)

        if second.x1 > x1 && second.x1 < x2 && first.y1 > y1 && first.y1 < y2 {
            return (second.x2, first.y2)
        }
    } else {
        let x1 = min(second.x1, second.x2)
        let x2 = max(second.x1, second.x2)

        let y1 = min(first.y1, first.y2)
        let y2 = max(first.y1, first.y2)
        
        if first.x1 > x1 && first.x1 < x2 && second.y1 > y1 && second.y1 < y2 {
            return (first.x1, second.y1)
        }
    }
...
var manhattan = 99999999999
var steps = 9999999999

for segmentFirst in first {
    for segmentSecond in second {
        if let (xi, yi) = checkSegmentIntersect(first: segmentFirst, second: segmentSecond) {
            print(segmentFirst)
            print(segmentSecond)
            ...
            let adjustedStepsFirst = segmentFirst.cumLen - abs(segmentFirst.x2 - xi) - abs(segmentFirst.y2 - yi)
            let adjustedStepsSecons = segmentSecond.cumLen - abs(segmentSecond.x2 - xi) - abs(segmentSecond.y2 - yi)
            
            let fullsteps = adjustedStepsFirst + adjustedStepsSecons
